...
import { ArrowLeft, Share2, Edit2, Download, Briefcase, Layout, DollarSign, Globe } from 'lucide-react';
    ...
    useEffect(() => {
        async function fetchWin() {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    setWin({ id: docSnap.id, ...docSnap.data() });
                }
            } catch (e) {
                console.error(e);
            } finally {
        ...
        // If mocking or waiting for real DB, uncomment above
        // For now, let's allow it to fetch real data
        if (id) fetchWin();
    }, [id]);
            ...
            <div className="sticky top-0 z-50 bg-[#0A0A0A]/95 backdrop-blur border-b border-[#222]">
                <div className="container mx-auto px-6 h-20 flex items-center justify-between">
                    ...
                    <div className="bg-primary text-black px-4 py-2 text-xs font-black uppercase tracking-widest rounded-sm">
                        Magnitude: {win.magnitude || 'High Impact'}
                    </div>
                </div>
            </div>
            ...
            <main className="container max-w-5xl mx-auto px-6 py-12 space-y-16">
                ...
                <section className="space-y-6">
                    ...
                    <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-widest text-muted-foreground">
                        <span>{win.date ? format(new Date(win.date), 'MMMM yyyy') : 'Date Unknown'}</span>
                ...
                {/* Core Impact Cards (Mocked Stat logic based on win data) */}
                <section className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-[#111] border border-[#222] p-8 rounded-sm">
                            ...
                            <div className="text-4xl font-bold text-white mb-1">{win.metric || "N/A"}</div>
                ...
                <section className="space-y-8 border-t border-[#222] pt-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-8 md:gap-16">
                        ...
                        <div className="text-lg text-white leading-relaxed text-opacity-90">
                            Legacy systems were causing significant downtime and scaling issues during peak traffic periods. Technical debt had reached a critical mass, preventing the deployment of new features.
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-8 md:gap-16">
                        ...
                        <div className="text-lg text-white leading-relaxed text-opacity-90">
                            {win.description || "Orchestrated a cross-functional team of 40 engineers to migrate services to AWS. Implemented IaC using Terraform and established a new CI/CD pipeline."}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-8 md:gap-16">
                        ...
                        <div className="text-lg text-white leading-relaxed text-opacity-90">
                            Achieved 99.99% uptime and reduced monthly infrastructure spend. The migration enabled the product team to scale to three new international markets.
                        </div>
                    </div>
                </section>
...
const StakeholderCard = ({ icon: Icon, role }: { icon: any, role: string }) => (
